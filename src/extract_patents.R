...
df <- read.csv(file.path("inst", "extdata", "Espacenet_search_result_20260829.csv"))
# colnames(df) <- df[1,]
# df <- df[-1,]
...
out = c(out, paste0("<h1>Selected Patents</h1>"))
out = c(out, "</a>")
out = c(out, "<hr>")
